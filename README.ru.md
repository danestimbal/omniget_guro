...
GuroHub — бесплатный загрузчик и набор медиаинструментов с открытым кодом для Windows, macOS и Linux.
...
<p align="center">
  <img src="assets/readme/hero.svg" alt="GuroHub: вставьте ссылку, получите файл. Загрузки, инструменты и учебная библиотека в одном приложении для Windows, macOS и Linux." width="100%" />
</p>
...
<p align="center">
  <sub>Русский перевод начал <a href="https://github.com/xJaroslav69">@xJaroslav69</a> (<a href="https://github.com/danestimbal/omniget_guro/pull/130">PR #130</a>). Спасибо.</sub>
</p>
...
<p align="center">
  <a href="https://github.com/danestimbal/omniget_guro/releases/latest"><img src="https://img.shields.io/github/v/release/danestimbal/omniget_guro?style=for-the-badge&label=release&color=F28500" alt="Последний релиз" /></a>
  <a href="https://github.com/danestimbal/omniget_guro/releases"><img src="https://img.shields.io/github/downloads/danestimbal/omniget_guro/total?style=for-the-badge&label=downloads&color=1E6FE8" alt="Всего загрузок" /></a>
  <a href="https://github.com/danestimbal/omniget_guro/stargazers"><img src="https://img.shields.io/github/stars/danestimbal/omniget_guro?style=for-the-badge&color=FFD426" alt="Звёзды на GitHub" /></a>
  ...
  <a href="https://hosted.weblate.org/engage/gurohub/"><img src="https://hosted.weblate.org/widget/gurohub/frontend-json/svg-badge.svg" alt="Статус перевода" /></a>
</p>

<p align="center">
  <a href="#скачать-и-установить"><img src="https://img.shields.io/badge/Скачать_для_Windows,_macOS_или_Linux-→-F28500?style=for-the-badge" alt="Скачать GuroHub" height="40" /></a>
  &nbsp;
...
<p align="center">
  <img src="assets/readme/home.png" alt="Главный экран GuroHub: вставьте URL, magnet-ссылку или .torrent, и файл окажется в вашей папке" width="900" />
</p>
...
- [Приватность и то, чего GuroHub не делает](#приватность-и-то-чего-omniget-не-делает)
...
GuroHub прячет всё это за одним текстовым полем. Вставьте ссылку, посмотрите превью с вариантами качества, нажмите скачать. То же окно потом воспроизводит курс, открывает PDF, транскрибирует аудио и делает бэкап доски Pinterest. yt-dlp и FFmpeg устанавливаются и обновляются сами, так что настраивать нечего и терминал открывать не нужно.

<p align="center">
  <img src="assets/readme/workflow.svg" alt="Как работает GuroHub: вставьте ссылку или нажмите горячую клавишу, GuroHub определяет сайт и скачивает через yt-dlp или собственный экстрактор, файл попадает в вашу папку и открывается во встроенном плеере, читалке или инструментах." width="100%" />
</p>

### Сравнение

| | GuroHub | Только yt-dlp | Веб-загрузчики для одного сайта | Платные загрузчики курсов |
|---|---|---|---|---|
...
yt-dlp — это движок, на котором работает GuroHub, и без него GuroHub бы не существовал. Если вы живёте в терминале и вам нужны только файлы, yt-dlp сам по себе подходит лучше.

---

## Скачать и установить

Выберите свою систему. Все сборки публикуются на [странице релизов](https://github.com/danestimbal/omniget_guro/releases/latest). Обновления приходят внутри приложения.
...
GuroHub не подписан платным сертификатом, поэтому каждая система показывает предупреждение при первом запуске. Для десктопных приложений с открытым кодом это нормально, и делается это один раз.

**Windows.** SmartScreen показывает синее окно. Нажмите **Подробнее**, затем **Выполнить в любом случае**.

**macOS.** Gatekeeper отказывается открывать приложение и может написать, что оно «повреждено». После того как перетащите GuroHub в Программы, откройте Терминал (Spotlight, наберите «Terminal») и вставьте эти две строки:
...
Затем откройте GuroHub из Launchpad как обычно.
...
1. Откройте GuroHub. Экран настройки спросит язык и тему, затем установит yt-dlp и FFmpeg одним кликом. yt-dlp проверяется по SHA-256 перед запуском.
2. Скопируйте любую ссылку: видео с YouTube, reel из Instagram, пост из X, доску Pinterest, magnet, прямую ссылку на файл.
3. Вставьте её в поле на главном экране. GuroHub определит сайт и покажет название, превью и доступные качества. Выберите и нажмите Enter.
...
<p align="center">
  <img src="assets/readme/downloads.png" alt="Страница загрузок GuroHub с активной 4K-загрузкой с YouTube: этап, скорость, оставшееся время и точная команда yt-dlp, плюс элементы в очереди и завершённые" width="900" />
</p>

### Вообще без окна

Скопируйте ссылку где угодно и нажмите **Ctrl+Shift+D** (**Cmd+Shift+D** на macOS). GuroHub прочитает буфер обмена и начнёт загрузку в фоне. Вторая клавиша, **Ctrl+Shift+M**, берёт только звук, так что ссылка на YouTube превращается в MP3, ничего не открывая. Она выключена, пока вы её не включите, а обе комбинации можно переназначить в **Настройки → Загрузки → Буфер обмена и горячие клавиши**.

---

## Что скачивает GuroHub

У GuroHub есть собственные экстракторы для самых популярных платформ, а всё остальное он передаёт [yt-dlp](https://github.com/yt-dlp/yt-dlp), который покрывает около 1 800 сайтов.
...
| Файлы и передача | `.torrent`-файлы и magnet-ссылки со встроенным BitTorrent-клиентом, прямые HTTP-файлы, манифесты HLS и DASH, а также передача между двумя установками GuroHub по короткому коду из слов |
...
<p align="center">
  <img src="assets/readme/settings.png" alt="Настройки GuroHub: сгруппированная боковая панель с разделами Внешний вид, Загрузки, Сеть, Cookies, Каналы, ИИ, Плагины и Дополнительно, и раздел загрузок с пунктами Вывод, Качество, Субтитры, Буфер обмена и горячие клавиши, Настройки платформ" width="900" />
</p>
...
Расширение делает две вещи. На сайтах, которые оно знает (YouTube, Instagram, TikTok, X, Reddit, Twitch, Pinterest, Bluesky, Telegram, Vimeo, Udemy, Hotmart, Rocketseat, Bilibili, SoundCloud), оно отправляет страницу в GuroHub одним кликом или по **Alt+O**. На любом другом сайте оно следит за сетевым трафиком, находит потоки MP4, HLS, DASH, WebM и аудио и показывает их во всплывающем окне. В обоих случаях оно передаёт ваши cookies и referer, и именно это позволяет GuroHub скачивать приватный контент, в который вы вошли: сторис Instagram, купленный курс, видео только для участников. Cookies группируются по реальному сайту, поэтому домен вида `.com.br` получает собственную запись, а не делит одну со всеми остальными `.com.br`. Во всплывающем окне есть и переключатель **Принудительный H.264** для YouTube, для компьютеров, которым тяжело с VP9 и AV1.

Выберите уровень, который вам удобен.

<p align="center">
  <img src="assets/readme/extension.svg" alt="Сопряжение: установите расширение в Chrome, нажмите «Сопрячь расширение» в настройках GuroHub, расширение находит приложение на localhost и подключается. Дальше каждая загрузка несёт ваши cookies." width="100%" />
</p>

### Уровень 1: из самого приложения (без скачиваний и zip)

1. Откройте GuroHub, установив его, если ещё не установили. Запустите хотя бы один раз.
2. Перейдите в **Настройки → Плагины → Расширение для браузера**. Нажмите **Обновить / Установить** напротив Chrome. GuroHub скопирует встроенное расширение в папку и откроет её.
...
5. Нажмите **Загрузить распакованное расширение** и выберите папку, которую только что открыл GuroHub.
6. Значок GuroHub появится на панели инструментов. Сама откроется страница параметров и сообщит, что ищет приложение.
7. Вернитесь в GuroHub, всё ещё в **Настройки → Плагины → Расширение для браузера**, и нажмите **Сопрячь расширение**. Через несколько секунд приложение напишет «Расширение подключено», а страница параметров станет зелёной. Готово.

Теперь на любой поддерживаемой странице нажимайте на значок. Страница, её cookies и заголовок уйдут в GuroHub, и загрузка начнётся. Cookies также появятся в **Настройки → Cookies**, где их используют плагин Courses и инструменты Instagram, X и Pinterest.

### Уровень 2: из zip-архива релиза

В каждом релизе есть `omniget-chrome-extension-vX.Y.Z.zip`. Скачайте его из [последнего релиза](https://github.com/danestimbal/omniget_guro/releases/latest), распакуйте и повторите шаги 3–7, указав в **Загрузить распакованное расширение** распакованную папку. Это удобно, если приложение стоит на одной машине, а браузер на другой, или вы ставите расширение кому-то ещё.
...
<table>
  <tr>
    <td><img src="assets/readme/tools-instagram.png" alt="Инструменты Instagram в GuroHub: скачать пост, пакетная загрузка, звук reel, сторис, актуальное, кто смотрел сторис, просмотр профиля, аватар в HD, скачать профиль, кто не подписан в ответ, фаны, взаимные, кто отписался, призрачные подписчики, белый список, экспорт данных, аналитика, сравнение профилей, хэштеги, комментарии, лайки, розыгрыш, публикация и планирование" /></td>
    <td><img src="assets/readme/tools-x.png" alt="Инструменты X / Twitter в GuroHub: скачать пост, развернуть тред, пост в картинку, рентген профиля, медиа профиля, расширенный поиск, экспорт закладок, кто не подписан в ответ, ваш архив X и Grok" /></td>
  </tr>
  <tr>
    <td><img src="assets/readme/tools-pinterest.png" alt="Инструменты Pinterest в GuroHub: скачать пин, бэкап доски, бэкап профиля, поиск без ИИ и рекламы, похожие пины, поиск источника, дубликаты, палитра, офлайн-галерея и идеи ключевых слов" /></td>
    <td><img src="assets/readme/tools-speech.png" alt="Инструменты речи и субтитров в GuroHub: транскрибация whisper.cpp, синтез речи, перевод субтитров, дубляж по субтитрам, а также запланированные клонирование голоса, дизайн голоса, выделение вокала и диктовка" /></td>
  </tr>
</table>
...
- **Отложенные посты.** Поставьте посты в очередь на дату и время. GuroHub публикует их, пока открыт. *бета*
...
- **Расходы на ИИ.** Сколько GuroHub потратил на ИИ по дням, моделям и задачам, из локального журнала.
...
- **MCP-сервер.** Инструменты GuroHub доступны по Model Context Protocol через локальный мост: 31 инструмент за тем же токеном, что у расширения, и готовые фрагменты конфигурации для Claude Code, Claude Desktop, Cursor, VS Code, Goose и Codex. *бета*
...
В GuroHub есть чат в стиле Discord под названием OmniDisc для серверов, которые вы поднимаете сами через [omnidisc-server](https://github.com/tonhowtf/omnidisc-server). Текстовые каналы, личные сообщения, друзья, роли и права, закреплённые сообщения, поиск, голос, видео и демонстрация экрана. Личные сообщения и файлы в них шифруются сквозным шифрованием по MLS, а ключ зашифрованного звонка выводится из той же группы, поэтому оператор сервера не может подслушать. Голос работает на Rust, а не в веб-вью, а демонстрация экрана использует аппаратный кодировщик машины. Файлы, отправленные через чат, шифруются на диске и удаляются с сервера через тридцать минут.
...
- Пересказ видео: вставьте URL в **Настройки → ИИ**, GuroHub возьмёт субтитры и сделает пересказ нужной длины и на нужном языке.
- Отправить файл человеку: выберите файл, поделитесь кодом из слов, получатель вставляет его в свой GuroHub.
...
## Приватность и то, чего GuroHub не делает

Всё работает на вашем компьютере. Нет аккаунта, нет нашего сервера посередине и нет телеметрии о том, что вы скачиваете. Cookies и API-ключи живут в вашем локальном профиле. Единственные сетевые запросы, которые GuroHub делает сам, идут к сайтам, откуда вы просили скачать, к GitHub за обновлениями и плагинами и к настроенному вами ИИ-провайдеру, когда вы пользуетесь ИИ-инструментом.

GuroHub скачивает то, что уже может открыть ваша собственная сессия. Он не обходит DRM, не ломает пейволлы и не передаёт учётные данные, а уроки с DRM пропускаются с уведомлением. Вы отвечаете за соблюдение авторских прав и условий каждой платформы. Полный текст в приложении: **О программе → Условия и этика**.
...
**GuroHub — это оболочка для yt-dlp?**
Частично. yt-dlp закрывает длинный хвост сайтов, а GuroHub его встраивает, проверяет и обновляет. Поверх этого — собственные экстракторы для курсов, Instagram, X, Pinterest, Bilibili, Telegram и торрентов, очередь с докачкой и повторами, раздел Tools и библиотека Study.
...
Debian и Ubuntu: `.deb`. Fedora, openSUSE, семейство RHEL: `.rpm`. Всё остальное: `.AppImage`. Публикуются и x86_64, и ARM64. GuroHub нет во Flathub.

---

## Командная строка

`omniget-cli` выходит с каждым релизом для Windows, macOS (Intel и Apple Silicon) и Linux. Возьмите `omniget-cli-<версия>-<платформа>` из [последнего релиза](https://github.com/danestimbal/omniget_guro/releases/latest).
...
Если вы просто хотите пользоваться GuroHub, [возьмите релиз](#скачать-и-установить). Для сборки нужны [Rust](https://rustup.rs/) (точная версия тулчейна закреплена в `rust-toolchain.toml`, потому что от неё зависит ABI плагинов), [Node.js](https://nodejs.org/) 18+ и [pnpm](https://pnpm.io/).
...
Сообщения об ошибках и pull request'ы — в [Issues](https://github.com/danestimbal/omniget_guro/issues) и [Pull requests](https://github.com/danestimbal/omniget_guro/pulls). Вопросы и быстрая помощь — в [Discord](https://discord.gg/jgdxyPy7Vn).

Переводы ведутся на [Weblate](https://hosted.weblate.org/engage/gurohub/). Выберите свой язык и переводите в браузере. Новые строки появляются там через несколько часов после попадания в `main`.

GuroHub построен на [yt-dlp](https://github.com/yt-dlp/yt-dlp), [FFmpeg](https://ffmpeg.org/), [gallery-dl](https://github.com/mikf/gallery-dl), [whisper.cpp](https://github.com/ggerganov/whisper.cpp), [aria2](https://aria2.github.io/), [SponsorBlock](https://sponsor.ajay.app/), [Return YouTube Dislike](https://returnyoutubedislike.com/), [FxTwitter](https://github.com/FixTweet/FxTwitter), [Spicetify](https://spicetify.app/) и [Tauri](https://tauri.app/). Спасибо всем, кто их поддерживает.

Loop, существо на главном экране, — маскот GuroHub. Фан-арт приветствуется. Оригинальную графику нельзя использовать в коммерческих целях и распространять в изменённом виде.

<p align="center">
  <a href="https://star-history.com/#danestimbal/omniget_guro&Date"><img src="https://api.star-history.com/svg?repos=danestimbal/omniget_guro&type=Date" alt="История звёзд danestimbal/omniget_guro" width="600" /></a>
</p>

<p align="center">
  <a href="https://github.com/danestimbal/omniget_guro/releases/latest"><b>Скачать GuroHub</b></a> · <a href="LICENSE">GPL-3.0</a>
</p>

<script lang="ts">
  ...
  const builtins: { kind: string; label: string; description: string }[] = [
    {
      kind: "daily-journal",
      label: "Daily journal",
      description: "Focus of the day, tasks, notes, reflection (with placeholders).",
    },
    {
      kind: "lesson-notes",
      label: "Lesson notes",
      description: "Structure for lesson notes with goals and questions.",
    },
    {
      kind: "book-highlights",
      label: "Book highlights",
      description: "Frontmatter with title/author + highlight sections.",
    },
    {
      kind: "weekly-review",
      label: "Weekly review",
      description: "Wins, blockers, learnings, next week's focus + a query of pending tasks.",
    },
    {
      kind: "concept-page",
      label: "Concept page",
      description: "Definition, why it matters, examples, related links, source.",
    },
  ];
  ...
  async function applyNow() {
    if (!applyTargetPageId) {
      showToast("err", "Choose the target page");
      return;
    }
    applying = true;
    try {
  ...
  async function markAsTemplate(pageId: number) {
    try {
      ...
      markPageOpen = false;
      await loadAll();
      showToast("ok", "Page marked as a template");
    } catch (e) {
      showToast("err", e instanceof Error ? e.message : String(e));
    }
  }
  ...
  async function unmarkAsTemplate() {
    ...
    try {
      ...
      selectedPreview = [];
      await loadAll();
      showToast("ok", "Page unmarked as a template");
    } catch (e) {
      showToast("err", e instanceof Error ? e.message : String(e));
    }
  }
  ...
  <aside class="left">
    ...
    <section>
      ...
      {#if templates.length === 0}
        <p class="empty">
          No pages marked as a template. Mark an existing page to reuse it on
          other pages.
        </p>
      {:else}
        <ul class="t-list">
...
{#if markPageOpen}
  ...
  >
    <div class="modal wide">
      <h3>Mark page as a template</h3>
      <p class="hint">
        The page becomes reusable. <code>&lt;%var%&gt;</code> in the content becomes
        placeholders filled in on apply.
      </p>
      <input
        type="text"
        placeholder="Search pages…"
...
<ConfirmDialog
  ...
  message="The page still exists, it just loses its template marking."
  confirmLabel="Unmark"
  variant="danger"

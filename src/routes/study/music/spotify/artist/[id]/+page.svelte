<script lang="ts">
    ...
    try {
      ...
      if (
        ...
        if (msg.includes("403") || msg.toLowerCase().includes("forbidden")) {
          error =
            "Spotify blocked access to this artist for apps in Development mode. You can play it by clicking one of their tracks from another screen.";
        }
      }
    } catch (e) {
  ...
  async function toggleFollow() {
    ...
    try {
      ...
      showToast("success", isFollowing ? "Following" : "Unfollow");
    } catch (e) {
      showToast("error", e instanceof Error ? e.message : String(e));
    } finally {
      followToggle = false;
  ...
  {#if artist}
    <header class="hero">
      ...
      <div class="hero-info">
        ...
        <h1>{artist.name}</h1>
        {#if artist.followers}
          <p class="meta">{artist.followers.toLocaleString()} followers</p>
        {/if}
        ...
        <div class="album-grid">
          {#each albums as a (a.id)}
            <a class="album-card" href={`/study/music/spotify/album/${a.id}`}>
              ...
              <p class="album-sub">{a.release_date ? a.release_date.slice(0, 4) : ""}{a.total_tracks ? ` · ${a.total_tracks} tracks` : ""}</p>
            </a>
          {/each}
        </div>
      </section>

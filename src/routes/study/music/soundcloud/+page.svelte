  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (soundcloudStore.isLoggedIn) {
        ...
        loginError = "Couldn't complete sign-in. Try again.";
      }
    } catch (e) {
...
<section class="sc-home">
  ...
  {:else if !soundcloudStore.isLoggedIn}
    <div class="login-card">
      ...
      <h2>Sign in with SoundCloud</h2>
      <p>A small SoundCloud window will open. Sign in and it closes on its own when you're done.</p>
      <div class="login-actions">
        <button type="button" class="cta" disabled={loginBusy} onclick={handleLogin}>
          {loginBusy ? "Opening SoundCloud…" : "Sign in with SoundCloud"}
        </button>
      </div>
      {#if loginError}
        <p class="error">{loginError}</p>
        <details class="why-fail">
          <summary>Why might this fail?</summary>
          <ul>
            <li>New account: confirm your email on SoundCloud first</li>
            <li>2FA: finish the second step before closing the window</li>
            <li>Captcha: solve it on the site, the window waits for you</li>
          </ul>
        </details>
      {/if}
    </div>

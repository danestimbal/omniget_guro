  ...
  {#if user}
    <header class="hero">
      ...
      <div class="info">
        ...
        <h1>{user.username}</h1>
        <p class="meta">
          {#if user.followers_count}{user.followers_count.toLocaleString()} followers{/if}
          ...
          {#each tracks.slice(0, 20) as track, i (track.id)}
            <div class="track-row" role="button" tabindex="0" onclick={() => play(i)} onkeydown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); play(i); } }}>
              ...
              <div class="m">
                ...
                {#if track.likes_count}<span class="a">♥ {track.likes_count.toLocaleString()}</span>{/if}
              </div>
        ...
        <div class="grid">
          {#each playlists as p (p.id)}
            <a class="card" href={`/study/music/soundcloud/playlist/${p.id}`}>
              ...
              <p class="card-sub">{p.track_count} tracks</p>
            </a>
          {/each}
        </div>
      </section>

<script lang="ts">
  ...
  function stageLabel(job: DownloadJobState): string {
    ...
    if (job.stage === "skipped") return "Already had it";
    if (job.stage === "done") {
      if (job.kind === "bulk") {
  ...
  async function retrySingle(job: DownloadJobState) {
    ...
    if (!dir) {
      showToast("error", "Pick the folder again from the download button.");
...
{#if hasJobs}
  <button
    ...
    aria-label={drawerOpen ? "Close downloads" : "Abrir downloads"}
            ...
            {#if canExpand}
              <div class="bulk-actions">
                ...
                >
                  ...
                  {failedList.length} {failedList.length === 1 ? "failed" : "failed"}
                </button>
                <button
                  type="button"
                  class="retry-all"

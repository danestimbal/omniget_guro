<script lang="ts">
  ...
  async function runTestVideo() {
    if (!testVideoId.trim()) {
      lastError = "Paste a video ID or YouTube URL.";
      return;
    }
    testRunning = true;
    lastError = null;
...
<section class="tab">
  <SettingsField label="YouTube client" description="State of the study plugin for YouTube/Music audio playback.">
  ...
  <SettingsField label="PoToken" description="Authentication token signed by YouTube. Generated automatically via bgutils-js in boa_engine.">
    {#if potokenStatus}
      <dl class="status-grid">
        <dt>Minting available</dt><dd><code>{potokenStatus.minting_available ? "yes" : "no"}</code></dd>
        <dt>Visitor token</dt><dd>{formatTokenStatus(potokenStatus.visitor)}</dd>
        <dt>Content tokens cached</dt><dd><code>{potokenStatus.content_cached_count}</code></dd>
      </dl>
    {/if}
    <div class="row">
    ...
    {#if manualPanelOpen}
      <div class="manual-panel">
        <p class="hint">
          Capture from DevTools (F12) on <code>music.youtube.com</code>: Network tab, POST request <code>youtubei/v1/player</code>, the body has
  ...
  <SettingsField label="Test video" description="Tries each client in the cascade and shows which one got a cipher/stream. Useful for debugging.">
    <div class="test-row">
      <input
        type="text"
        placeholder="Paste a video ID (dQw4w9WgXcQ) or full URL"
    ...
    {#if testResult}
      <div class="test-summary">
        ...
        {#if testResult.first_working_client}
          first working client: <code>{testResult.first_working_client}</code>
        {:else}
